...
void GCS_MAVLINK::uploadFont(mavlink_channel_t chan)
{
	...
	hal.console->printf_P(PSTR("Ready for Font\n"));

	while(font_count < 256) { 
		uint8_t incomingByte = comm_receive_ch(chan);
		...
		if(byte_count == 64)
		{  
			osdMax7456.write_NVM(font_count, character_bitmap);
			byte_count = 0;
			font_count++;
			hal.console->printf_P(PSTR("Char Done\n"));
		}
	}
